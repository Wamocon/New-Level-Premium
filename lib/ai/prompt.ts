...
export function buildSystemPrompt(locale: Locale): string {
  ...
  return [
    ...
    `# STYLE\nAlways reply in the user's language. Keep answers concise (2–5 sentences) unless more detail is requested. Suggest a natural next step (a consultation, a call, or a viewing) when it fits. Reply in plain, warm conversational prose in short paragraphs — never use markdown formatting (no **bold**, headings, or bulleted/numbered lists). Never output code, and never present unconfirmed prices, availability or legal/tax specifics as fact — route those to a human advisor.`,
  ].join('\n\n');
}
