...
function Cluster({ reduced }: { reduced: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    g.rotation.x = THREE.MathUtils.lerp(g.rotation.x, py * 0.5, 0.06);
    g.position.x = THREE.MathUtils.lerp(g.position.x, px, 0.06);
  });
  return (
    <group ref={group}>
      {TOWERS.map((t, i) => (
...
function Dust({ reduced }: { reduced: boolean }) {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial size={0.035} color={GOLD} transparent opacity={0.55} sizeAttenuation depthWrite={false} />
    </points>
  );
}
...
export default function HeroCanvas() {
  ...
  // Pause all rendering when the hero is scrolled off-screen or the tab is hidden.
  useEffect(() => {
    ...
    io.observe(el);
    const onVis = () => setInView(!document.hidden && document.visibilityState === 'visible');
